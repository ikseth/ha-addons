...
func TestPayloadIncludesDisabledUpdatesAndCorrectsWindowsElevenName(t *testing.T) {
	...
	if data["updates_state"] != "disabled" || data["updates_provider"] != "disabled" {
		t.Fatalf("unexpected update block: %#v", data)
	}
}
